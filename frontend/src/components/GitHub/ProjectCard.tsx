        ...
        <div class="flex items-start justify-between gap-4">
          <div class="flex-1 min-w-0">
            ...
            <div class="flex items-center gap-2 mb-2">
              <h3 class="font-sans text-lg text-neutral-100 truncate group-hover:text-cyan-400 transition-colors duration-200">
                {repo.name}
              </h3>

              <div class="flex items-center gap-1">
                <Show when={repo.is_private}>
                  <span class="px-1.5 py-0.5 bg-yellow-900/30 text-yellow-400 border border-yellow-800 rounded text-xs font-sans">
                    PRIVATE
                  </span>
                </Show>
                <Show when={repo.is_fork}>
                  <span class="px-1.5 py-0.5 bg-blue-900/30 text-blue-400 border border-blue-800 rounded text-xs font-sans">
                    FORK
                  </span>
                </Show>
                <Show when={repo.is_archived}>
                  <span class="px-1.5 py-0.5 bg-neutral-800 text-neutral-500 border border-neutral-700 rounded text-xs font-sans">
                    ARCHIVED
                  </span>
                </Show>
              </div>
          ...
          <div class="flex flex-col items-end gap-2 text-xs">
            <div
              class={`px-2 py-1 rounded font-sans ${
                healthMetrics.healthRating === 'excellent'
                  ? 'bg-green-900/30 text-green-400'
                  : healthMetrics.healthRating === 'good'
                    ? 'bg-blue-900/30 text-blue-400'
                    : healthMetrics.healthRating === 'fair'
            ...
            <div class="text-neutral-600 font-sans">
        ...
        <div class="flex items-start justify-between mb-3">
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 mb-1">
              <h3 class="font-sans text-xl text-neutral-100 truncate group-hover:text-cyan-400 transition-colors duration-200">
                {repo.name}
              </h3>
            </div>
            ...
            <div class="flex items-center gap-1 mb-2">
              ...
              <Show when={repo.is_archived}>
                <div
                  class="w-2 h-2 bg-neutral-600 rounded-full"
          ...
          {/* Health indicator */}
          <div
            class={`px-2 py-1 rounded text-xs font-sans ${
              healthMetrics.healthRating === 'excellent'
        ...
        <Show when={repo.topics.length > 0}>
          <div class="flex flex-wrap gap-1 mb-4">
            <For each={repo.topics.slice(0, 3)}>
              {(topic) => (
                <span class="text-xs px-2 py-1 bg-neutral-800/50 text-neutral-400 rounded font-sans border border-neutral-700">
                  {topic}
                </span>
              )}
            </For>
            <Show when={repo.topics.length > 3}>
              <span class="text-xs px-2 py-1 text-neutral-600 font-sans">
          ...
          <div class="w-full h-1 bg-neutral-800 rounded-full mt-1 overflow-hidden">
            <div
              class={`h-full rounded-full transition-all duration-500 ${
                healthMetrics.activityScore > 70
                  ? 'bg-green-500'
                  : healthMetrics.activityScore > 40
                    ? 'bg-yellow-500'
                    : 'bg-red-500'
              }`}
              style={{
                width: `${Math.min(healthMetrics.activityScore, 100)}%`,
              }}
            ></div>
          </div>
        </div>
