...
export const RustMetrics: Component = () => {
  ...
  const getCategoryColor = (category: string): string => {
    switch (category) {
      ...
      default:
        return 'border-neutral-500/30 text-neutral-400';
    }
  };
    ...
    try {
      ...
      const newMetrics: RustMetric[] = [
        {
          ...
          trend: calculateTrend(
            appData.average_response_time_ms || 0,
            getPreviousValue('Response Time'),
          ),
        ...
        },
        {
          ...
          unit: 'req/s',
          trend: calculateTrend(
            (appData.requests_handled || 0) / 60,
            getPreviousValue('Requests/sec'),
          ),
        ...
        },
        {
          ...
          unit: '',
          trend: calculateTrend(
            sysData.active_processes || 0,
            getPreviousValue('Active Connections'),
          ),
        ...
        },
        {
          name: 'Memory Usage',
          value:
            appData.memory_usage_mb || sysData.memory_usage_percent || 0,
          unit: appData.memory_usage_mb ? 'MB' : '%',
          trend: calculateTrend(
            appData.memory_usage_mb || sysData.memory_usage_percent || 0,
            getPreviousValue('Memory Usage'),
          ),
        ...
        },
        {
          ...
          unit: '%',
          trend: calculateTrend(
            sysData.cpu_usage_percent || 0,
            getPreviousValue('CPU Usage'),
          ),
        ...
        },
        {
          ...
          unit: '%',
          trend: calculateTrend(
            appData.cache_hit_rate || 0,
            getPreviousValue('Cache Hit Rate'),
          ),
        ...
        },
        {
          ...
          unit: '',
          trend: calculateTrend(
            appData.database_connections || 0,
            getPreviousValue('Database Connections'),
          ),
      ...
      if (data.heap_allocated_mb !== undefined) {
        newMetrics.push({
          ...
          unit: 'MB',
          trend: calculateTrend(
            data.heap_allocated_mb,
            getPreviousValue('Heap Allocated'),
          ),
  ...
  const getHealthStatus = (): { status: string; color: string } => {
    ...
    const errorRate =
      (metrics().find((m) => m.name === 'Error Rate')?.value as number) || 0;
    const responseTime =
      (metrics().find((m) => m.name === 'Response Time')?.value as number) ||
      0;
    const cpuUsage =
      (metrics().find((m) => m.name === 'CPU Usage')?.value as number) || 0;
    ...
    <div class="space-y-6">
      ...
      {metrics().length === 0 ? (
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {metrics().map((metric) => (
            <div
              class={`glass-effect-strong border rounded-lg p-4 hover-lift transition-all duration-500 ${getCategoryColor(metric.category)}`}
            >
              <div class="flex items-center justify-between mb-2">
                ...
                <span
                  class={`text-sm ${getTrendColor(metric.trend, metric.category)}`}
                >
                      ...
                      width: `${Math.min(
                        ...
                        Math.max(
                          ...
                          metric.name === 'Response Time'
                            ? Math.min(
                                100,
                                ((metric.value as number) / 100) * 100,
                              )
      ...
      {metrics().length > 0 && (
        <div class="glass-effect border border-neutral-700/50 rounded-lg p-4">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            {['performance', 'memory', 'concurrency', 'throughput'].map(
              (category) => {
                ...
                const avgTrend =
                  categoryMetrics.length > 0
                    ? categoryMetrics.filter((m) => m.trend === 'up')
                        .length / categoryMetrics.length
                    : 0;

                return (
                  <div>
                    <div
                      class={`text-lg font-mono font-light ${getCategoryColor(category).split(' ')[1]}`}
                    >
