...
export const BenchmarkChart: Component = () => {
  ...
  const runSystemBenchmark = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'System benchmark failed',
      );
    } finally {
      setIsRunning(false);
      setTimeout(() => {
    ...
    return [
      {
        ...
        results: results.benchmark_results.map((result) => ({
          ...
          comparison: calculateComparison(
            result.mandelbrot.pixels_per_ms,
            getBaselinePixelsPerMs(result.resolution),
          ),
        })),
      },
      {
    ...
    return [
      {
        ...
        current:
          system.benchmarks?.cpu?.multi_thread?.primes_per_second || 0,
      ...
      },
      {
        ...
        current:
          system.benchmarks?.memory?.sequential_read?.mb_per_second || 0,
    ...
    <div class="space-y-6">
      ...
      <div class="text-center mb-8">
        ...
        <p class="text-neutral-500 max-w-2xl mx-auto">
          Comprehensive performance testing demonstrating computational
          efficiency across mathematical operations, system resources, and
      ...
      <div class="grid md:grid-cols-2 gap-4 mb-8">
        ...
        >
          ...
          <div class="text-sm text-neutral-400 mb-3">
            Test mathematical computation performance with parallel
            processing
          </div>
      ...
      <Show when={isRunning()}>
        <div class="bg-neutral-900/50 border border-neutral-800 rounded-lg p-6">
          <div class="flex items-center justify-between mb-3">
            <div class="text-neutral-300 font-mono text-sm">
              {currentTest()}
            </div>
      ...
      <Show when={benchmarkResults()}>
        <div class="space-y-6">
          <h3 class="text-xl font-mono text-neutral-300">
          ...
          <For each={getBenchmarkCategories()}>
            {(category) => (
              <div class="bg-neutral-900/30 border border-neutral-800 rounded-lg p-6">
                <div class="mb-4">
                  <h4 class="text-lg font-mono text-neutral-300 mb-1">
                    {category.name}
                  </h4>
                  ...
                  <For each={category.results}>
                    {(result) => (
                      <div class="bg-neutral-800/30 border border-neutral-700 rounded-sm p-4">
                        ...
                        <div
                          class={`text-xs mb-2 ${getPerformanceColor(result.performance_rating)}`}
                        >
                          {result.performance_rating}
                        </div>
                        <Show when={result.comparison !== undefined}>
                          <div
                            class={`text-xs ${getComparisonColor(result.comparison!)}`}
                          >
                            {result.comparison! > 0 ? '+' : ''}
                            {result.comparison!.toFixed(1)}% vs baseline
                          </div>
                        </Show>
                      </div>
                    )}
          ...
          <div class="bg-neutral-900/30 border border-neutral-800 rounded-lg p-6">
            ...
            <div class="grid md:grid-cols-3 gap-6 text-sm">
              ...
              <div>
                ...
                <div class="text-neutral-300 font-mono">
                  {
                    benchmarkResults()!.performance_analysis
                      .optimization_level
                  }
                </div>
                <div class="text-neutral-600 text-xs">
      ...
      <Show when={systemBenchmark()}>
        <div class="space-y-6">
            ...
            <For each={getSystemComparisons()}>
              {(comparison) => (
                <div class="bg-neutral-900/30 border border-neutral-800 rounded-lg p-6 text-center">
                  ...
                  <div
                    class={`text-sm ${getComparisonColor(calculateComparison(comparison.current, comparison.baseline))}`}
                  >
                    {calculateComparison(
                      comparison.current,
                      comparison.baseline,
          ...
          <div class="text-center p-6 bg-neutral-800/30 rounded-lg">
            ...
            <div
              class={`text-3xl font-mono ${getPerformanceColor(systemBenchmark().performance_rating)}`}
            >
      ...
      <Show when={!benchmarkResults() && !systemBenchmark() && !isRunning()}>
        <div class="text-center py-12">
          ...
          <div class="text-xl text-neutral-400 mb-2">
            No Benchmark Results
          </div>
          <div class="text-sm text-neutral-600">
            Run a benchmark to see detailed performance analysis and
            comparison
          </div>
        </div>
      </Show>
