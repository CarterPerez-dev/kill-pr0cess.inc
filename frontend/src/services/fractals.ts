...
class FractalService {
  ...
  async generateMandelbrot(params: {
    ...
    const request: FractalRequest = {
      ...
      max_iterations:
        ...
        this.calculateOptimalIterations(params.zoom),
    };
  ...
  async generateJulia(params: {
    ...
    const request: FractalRequest = {
      ...
      max_iterations:
        params.max_iterations ||
        this.calculateOptimalIterations(params.zoom, 'julia'),
    };
  ...
  private async makeRequest(
    ...
    try {
      ...
      const response = await apiClient.post<FractalResponse>(
        url,
        {},
        { timeout },
      );
  ...
  private validateFractalResponse(
    response: any,
  ): response is FractalResponse {
    return (
      response &&
  ...
  private calculateOptimalIterations(
    zoom: number,
    type: 'mandelbrot' | 'julia' = 'mandelbrot',
  ): number {
  ...
  private calculateTimeout(request: FractalRequest): number {
    ...
    const complexityTimeout =
      (pixelCount / 100000) * complexityFactor * zoomComplexity * 1000;
  ...
  analyzePerformance(response: FractalResponse): {
    ...
    const efficiency =
      performance_metrics.parallel_efficiency > 0.8
        ? 'Excellent'
        : performance_metrics.parallel_efficiency > 0.6
          ? 'Good'
          : performance_metrics.parallel_efficiency > 0.4
            ? 'Fair'
            : 'Poor';
    ...
    if (performance_metrics.parallel_efficiency < 0.6) {
      recommendations.push(
        'Consider reducing resolution for better parallel efficiency',
      );
    }

    if (performance_metrics.memory_usage_mb > 100) {
      recommendations.push(
        'High memory usage detected - consider lower iteration count',
      );
    }

    if (pixelsPerSecond < 1000) {
      recommendations.push(
        'Performance below optimal - try lower zoom or iteration count',
      );
    }
  ...
  getCacheStats() {
    ...
    return {
      ...
      validEntries: entries.filter(
        ([_, entry]) => now - entry.timestamp <= this.CACHE_TTL,
      ).length,
      memoryUsage: entries.reduce(
        (total, [_, entry]) => total + entry.data.data.length * 4,
        0,
      ), // Approximate bytes
