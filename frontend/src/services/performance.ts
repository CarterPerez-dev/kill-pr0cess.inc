...
class PerformanceService {
  ...
  async getCurrentMetrics(): Promise<PerformanceSnapshot> {
    ...
    try {
      const response = await apiClient.get<PerformanceSnapshot>(
  ...
  compareWithHistorical(
    ...
    historicalPeriodMinutes: number = 60,
  ): {
    ...
    const avgCpu =
      historicalMetrics.reduce((sum, m) => sum + m.cpu_usage_percent, 0) /
      historicalMetrics.length;
    const avgMemory =
      historicalMetrics.reduce((sum, m) => sum + m.memory_usage_percent, 0) /
      historicalMetrics.length;
    ...
    return {
      cpu_trend:
        cpuDelta > 5 ? 'degrading' : cpuDelta < -5 ? 'improving' : 'stable',
      memory_trend:
        memoryDelta > 5
          ? 'degrading'
          : memoryDelta < -5
