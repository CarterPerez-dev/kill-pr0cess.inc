...
class ApiClient {
  ...
  private recordFailure(endpoint: string) {
    ...
    if (circuit.failures >= 2) {
      circuit.isOpen = true;
      console.warn(
        `Circuit breaker opened for ${endpoint} after ${circuit.failures} failures`,
      );
    }
  ...
  private async executeRequest<T>(
    ...
    if (!this.checkCircuitBreaker(endpoint)) {
      throw new Error(
        `Circuit breaker is open for ${endpoint}. Too many recent failures.`,
      );
    }
  ...
  async healthCheck(): Promise<{ status: string; timestamp: string }> {
    try {
      const health = await this.get<{ status: string; timestamp: string }>(
        '/health',
      );
      return health;
    } catch (error) {
